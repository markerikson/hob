...
const LiveMenu: React.FC<FooterMenuProps> = ({match}) => {
  ...
  function renderSubMenus(menus: Submenu[]) {
    return menus.map((r: Submenu, index) => (
      <IonItem key={r.resource} href={r.resource} disabled={false} class="ion-margin-vertical">
        <img src={r.inactive_icon} alt={t(r.name.toString())} width='23%' height='auto' max-height='250px'/>
